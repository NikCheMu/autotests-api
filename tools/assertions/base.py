...
def assert_status_code(actual: int, expected: int):
    ...
    assert actual == expected, f"Actual status code: {actual}, Expected {expected}"
...
def assert_equal(actual: Any, expected: Any, name: str):
    ...
    assert actual == expected, f"Incorrect value of {name}. Actual {actual}, Expected {expected}"
...
def assert_length(actual: Sized, expected: Sized, name: str):
    ...
    with allure.step(f"Check that length of {name} equals to {len(expected)}"):
        logger.info(f"Check that {name} length is {expected}")
        assert len(actual) == len(expected), f"Incorrect length of {name}. Actual {len(actual)}, Expected {len(expected)}"