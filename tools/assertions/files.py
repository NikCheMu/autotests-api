...
from clients.errors_schema import ValidationErrorResponseSchema, ValidationErrorsSchema, InternalErrorResponseSchema
from clients.files.files_schema import UploadFileRequestSchema, UploadFileResponseSchema, FileSchema, \
...
def assert_get_file_with_incorrect_file_id_response(actual: ValidationErrorResponseSchema):
    """
    Проверяет, что ответ на получение файла с пустым некорректным id соответствует ожидаемой валидационной ошибке.
    ...
    expected = ValidationErrorResponseSchema(
        details=[
            ValidationErrorsSchema(
                ...
                context={"error": "invalid character: expected an optional prefix of `urn:uuid:` followed by [0-9a-fA-F-], found `i` at 1"},  # Минимальная длина строки должна быть 1 символ.
